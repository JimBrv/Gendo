...
{
#ifdef UNICODE
	Hinternet_OpenUrl_ = InternetOpenUrl(Hinternet_Main_, (LPCWSTR)(strUrl), NULL, 0, nFalg, 0);
#else
	Hinternet_OpenUrl_ = InternetOpenUrl(Hinternet_Main_, (LPCSTR)(strUrl), NULL, 0, nFalg, 0);
#endif
...
BOOL _Nv_Download::Download(__in CONST CHAR *strUrl, __in CONST CHAR *strFileSavePath, __in CONST CHAR *strCoolie, __in BOOL bModeFalg)
{
	...
	if (Http_Return_206 == Nv_Task_->Detece_ThreadMode(strUrl, 0, 0,  strCoolie))
	{
		...
		if (n64FileSize < n64ThreadDownloadCnt_ * 5)
			...
			dwDownloadVectorSize_ = (int)((n64FileSize / n64ThreadDownloadCnt_) * 2);
		...
		{
			for (int i=0; i<(int)dwDownloadVectorSize_; ++i)
			{
				nDownloadCount_[i] = 0;
			}
		}
...
__int64 _Nv_Download::GetDownloaded(VOID)
{
    if (bThreadMode_ == FALSE)
        return n64DownloadCount_;
    ...
    DWORD dwSize = vtThread_Task_.size();
    for (int i=0; i<nDataCount_; ++i) {
        _Nv_Download_Task *pTask = vtThread_Task_[i];
        if (pTask) 
            n64Ret += pTask->m_iGot;
    }
    n64DownloadCount_ = n64Ret;
...
{
	HKEY hKey;
	::RegOpenKeyEx(HKEY_CURRENT_USER,_T("Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings"),0,KEY_WRITE,&hKey);
	::RegSetValueEx(hKey,_T("MaxConnectionsPer1_0Server"),0,REG_DWORD,(CONST BYTE *)&dwCount, 4);
	::RegSetValueEx(hKey,_T("MaxConnectionsPerServer"),0,REG_DWORD,(CONST BYTE *)&dwCount, 4);
	::RegCloseKey(hKey);
}
...
BOOL _Nv_Download::read_DwonloadFile(VOID)
{
	...
	if (fp != NULL)
	{
		...
		//Pangtou 2012.12.8
		//return DeleteFile((LPCSTR)(strDownloadFile_));
#ifdef UNICODE
		return DeleteFile((LPCWSTR)(strDownloadFile_.c_str()));
#else
		return DeleteFile((LPCSTR)(strDownloadFile_.c_str()));
#endif // !UNICODE

...
void CControlUI::DoPaint(HDC hDC, const RECT& rcPaint)
{
	...
	if (m_pControlUIEvent){
		const RECT &rc = GetPos();
		m_pControlUIEvent->DoPaint(hDC,rc);
    }
}
...
void CControlUI::SetCControlUIEventHandler( CControlUIEventHandler* pEventHandler )
{
	if ( pEventHandler!=NULL  && m_pControlUIEvent!=pEventHandler)
		m_pControlUIEvent=pEventHandler;
}

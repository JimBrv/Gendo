...
namespace DuiLib
{
	...
	CControlUI* CContainerUI::FindControl(FINDCONTROLPROC Proc, LPVOID pData, UINT uFlags)
	{
		...
		if( (uFlags & UIFIND_TOP_FIRST) != 0 ) {
            ...
            int cnt = m_items.GetSize();
			for( int it = 0; it < cnt; it++ ) {
				CControlUI* pControl2 = static_cast<CControlUI*>(m_items[it]);
                /* bug fix segment for DUI, gendo 2013-07-24 */
                if (pControl2 == NULL) {
                    return NULL;
                }
                CControlUI* pControl = pControl2->FindControl(Proc, pData, uFlags);
				if( pControl != NULL ) {
					if( (uFlags & UIFIND_HITTEST) != 0 && !pControl->IsFloat() && !::PtInRect(&rc, *(static_cast<LPPOINT>(pData))) )
						continue;
					else 

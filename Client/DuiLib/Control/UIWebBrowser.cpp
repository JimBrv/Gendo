...
IHTMLWindow2* DuiLib::CWebBrowserUI::GetHTMLWindow2()
{
	...
	NULLTEST( pHD2 );
	HRTEST_SE( pHD2->get_parentWindow(&_pHtmlWnd2),L"IHTMLWindow2::get_parentWindow Failed" );
	return _pHtmlWnd2;
RETURN:
	return NULL;
}
...
IHTMLDocument2* DuiLib::CWebBrowserUI::GetHTMLDocument2()
{
	if( _pHtmlDoc2 != NULL )
		return _pHtmlDoc2;
	IDispatch* pDp =  NULL;
	HRTEST_SE(m_pWebBrowser2->get_Document(&pDp),L"DWebBrowser2::get_Document Failed");
...
DuiLib::CDuiString DuiLib::CWebBrowserUI::GetCurURL() const
{
	return m_strURL;
}
